...
export const useCustomerStore = defineStore('customer', () => {
  ...
  async function fetchCustomerById(id) {
    try {
      ...
      const transformedAddresses = (apiCustomer.addresses || []).map(
        (addr) => ({
          ...addr,
          province:
            customerStore.provinces.find((p) => p.id === addr.provinceId) ||
            null,
      ...
      currentCustomer.value = {
        ...
        avatar:
          apiCustomer.avatar || 'https://cdn.vuetifyjs.com/images/john.jpg',
  ...
  async function fetchCustomerOrders(id) {
    try {
      ...
      if (customer) {
        ...
        customer.totalSpent = currentCustomerOrders.value.reduce(
          (sum, o) => sum + o.total,
          0
        )
        customer.lastOrderDate =
          currentCustomerOrders.value.length > 0
            ? new Date(
                Math.max(
                  ...currentCustomerOrders.value.map((o) =>
  ...
  const avgOrder = computed(() => {
    ...
    return (
      customerStats.value.totalSpent / customerStats.value.ordersCount
    ).toFixed(2)
  })
