        ...
        <v-window v-model="tab">
                ...
                <v-card class="mb-4" elevation="2">
                  ...
                  <v-card-text>
                    <v-chip-group>
                      ...
                      <v-chip v-if="customerSegments.length === 0" color="grey">
          ...
          <v-window-item value="segments">
            <v-card class="ma-2" elevation="2">
              ...
              <v-card-text>
                <v-alert v-if="customerSegments.length === 0" type="info">
                  This customer doesn't belong to any segments.
                </v-alert>
                ...
                <h3 class="text-h6 mb-2">Available Segments</h3>
                <v-data-table
                ...
                >
                  ...
                  <template v-slot:item.actions="{ item }">
                    <v-btn
                      color="primary"
                      size="small"
                      @click="addToSegment(item)"
                      :disabled="isInSegment(item.id)"
                    >
                      <v-icon left>mdi-plus</v-icon>
                      Add
                    </v-btn>
...
const segmentHeaders = [
  { title: 'Segment Name', key: 'name' },
  { title: 'Criteria', key: 'criteria' },
  { title: 'Customer Count', key: 'customerCount' },
  { title: 'Actions', key: 'actions', sortable: false }
]
...
const allSegments = computed(() => {
  return customerStore.segments.map(segment => {
    const customersInSegment = customerStore.getCustomersInSegment(segment.id)
    return {
      ...segment,
      description: formatCriteria(segment.criteria),
...
const customerSegments = computed(() => {
  ...
  return customerStore.segments.filter(segment => {
    return customerStore.evaluateCustomerForSegments(customer.value.id).includes(segment.id)
  })
})
...
const getSegmentColor = (segment) => {
  const colors = {
    'High Value': 'amber',
    'Frequent Buyers': 'blue',
    'Inactive': 'grey',
    'New Customers': 'green'
  }
  return colors[segment.name] || 'primary'
}
...
const isInSegment = (segmentId) => {
  return customerSegments.value.some(s => s.id === segmentId)
}

const addToSegment = (segment) => {
  // In a real app, this would modify the segment criteria
  toast.info(`Customer added to segment ${segment.name}`)
}
</script>
