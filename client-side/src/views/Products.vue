    ...
    >
      <template v-slot:[`item.name`]="{ item }">
        <div class="d-flex align-center product-cell">
          ...
          <span class="product-name ml-1">{{ item.name }}</span>
        </div>
      </template>

      <template v-slot:[`item.sku`]="{ item }">
        <span>{{ item.colors && item.colors[0] ? item.colors[0].sku : 'N/A' }}</span>
      </template>

      <template v-slot:[`item.categories`]="{ item }">
        <span>{{ item.categories && item.categories[0] ? item.categories[0].name : 'N/A' }}</span>
      </template>

      <template v-slot:[`item.colors`]="{ item }">
        <span>{{ item.colors ? item.colors.reduce((sum, color) => sum + color.quantity, 0) : 0 }}</span>
      </template>
