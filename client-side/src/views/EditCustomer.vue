                ...
                <v-row :class="{ 'flex-row-reverse': locale === 'ar' }">
                  <v-col cols="12" md="6">
                    ...
                    <v-select
                      ...
                      :error-messages="
                        address.province ? undefined : $t('invalidProvince')
                      "
                    />
                  </v-col>
                  <v-col cols="12" md="6">
...
<script setup>
        ...
        addresses:
          ...
          existingCustomer.addresses.length > 0
            ? existingCustomer.addresses.map((addr) => ({
                ...
                province: addr.provinceId
                  ? customerStore.provinces.find(
                      (p) => p.id === Number(addr.provinceId)
                    ) || null
                  : null,
                city: addr.cityId
                  ? customerStore.cities.find(
                      (c) => c.id === Number(addr.cityId)
                    ) || null
                  : null
              }))
            : []
  ...
  const saveCustomer = async () => {
    ...
    try {
      ...
      const hasValidAddress = customer.value.addresses.some(
        (addr) => addr.province?.id && addr.city?.id
      )
      ...
      await customerStore.updateCustomer(
        customer.value.id || customer.value._id,
        customerData
      )
